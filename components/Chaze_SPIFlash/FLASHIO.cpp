...
bool SPIFlash::_transferAddress(void) {
  address4ByteEnabled = true;			//von mir (Constantin Koch) hinzugef�gt
...
bool SPIFlash::_notBusy(uint32_t timeout) {
  ...
  do {
    ...
    //_time++;											//##########################################################  von mir (Constantin Koch) auskommentiert
  } while ((micros() - _time) < timeout);
  if (timeout <= (micros() - _time)) {
	  //Clear Status Register 1							//##########################################################  von mir (Constantin Koch) hinzugef�gt
