...
bool SPIFlash::begin(uint8_t cs, uint32_t flashChipSize) {
  csPin = cs;
  pinMode(csPin, OUTPUT);
  CHIP_DESELECT
...
bool SPIFlash::writeByteArray(uint32_t _addr, uint8_t *data_buffer, size_t bufferSize, bool errorCheck) {
  ...
  if (bufferSize <= maxBytes) {
    ...
    do {
      ...
      length -= writeBufSz;
      maxBytes = 512;   // Now we can do up to 512 bytes per loop	//von mir selbst (Constantin Koch) von 256 auf 512 ge�ndert, obwohl er hier eigentlich nie hinkommen sollte
...
// Erases one 256k block.
//  Takes an address as the argument and erases the block containing the address.
bool SPIFlash::eraseBlock256K(uint32_t _addr) {
#ifdef RUNDIAGNOSTIC
  _spifuncruntime = micros();
#endif
  ...
  /*if(!_notBusy(1200 * 1000L)) {										//von mir (Constantin Koch) auskommentiert, da er erstmal weitergehen und sp�ter abgefragt werden soll
    return false;	//Datasheet says erasing a sector takes 400ms max
    }*/
#ifdef RUNDIAGNOSTIC
  _spifuncruntime = micros() - _spifuncruntime;
...
uint8_t SPIFlash::CheckErasing_inProgress() {							//von mir (Constantin Koch) hinzugef�gt
  _readStat1();										//stat1: 0=ready
  if (stat1 != 0 && stat1 != 3) {
    //Clear Status Register 1
    _beginSPI(0x30);  //CLSR (Clear Status Register)
    _endSPI();
  }
  return stat1;										//       3=erasing
}														//    else=error
