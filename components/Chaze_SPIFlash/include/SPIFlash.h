...
// Defines and variables specific to ARM architecture
  ...
  #define CHIP_DESELECT digitalWrite(csPin, HIGH);
  #define xfer(n)   SPI.transfer(n)
  #define BEGIN_SPI SPI.begin(SPI_SCK, SPI_MISO, SPI_MOSI, -1);
#endif
...
public:
  ...
  SPIFlash(uint8_t cs, SPIClass *spiinterface=&SPI);
  #elif defined (BOARD_RTL8195A)
  SPIFlash(PinName cs);
  #else
  SPIFlash(uint8_t cs);
  ...
  bool     begin(uint8_t cs, uint32_t flashChipSize = 0);
  ...
  bool     eraseBlock256K(uint32_t _addr);		//Diese Funktion wurde von mir selbst (Constantin Koch) von eraseBlock64 zu eraseBlock256 umbenannt, da 256kB unsere kleinste Sektorsize ist und das entprechende Command auch einen 256kB-Sektor erased
  uint8_t  CheckErasing_inProgress();				//Diese Funktion wurde von mir selbst (Constantin Koch) hinzugef�gt		0=ready
  bool     eraseChip(void);																					                                    //				3=erasing				
  //-------------------------------- Power functions ------------------------------------    //								 else=error
...
//#endif
  ...
  //################################################# Der folgende Teil wurde von mir selbst (Constantin Koch) hinzugefuegt
  bool	  ASP_PPB_protect_sector(uint32_t _byteaddr);
  uint8_t ASP_PPB_read_Access_Register(uint32_t _byteaddr);    //0xFF=not protected   0x00=protected   other=error
  bool	  ASP_release_all_sectors(void);
  //#################################################
...
//      Use the eraseSector()/eraseBlock32K/eraseBlock256K commands to first clear memory (write 0xFFs)
